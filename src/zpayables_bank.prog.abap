...
*      lfa1~ktokk
 ...
 INTO CORRESPONDING FIELDS OF TABLE ITAB
                             FROM BSIK INNER JOIN LFB1 ON LFB1~LIFNR = BSIK~LIFNR AND LFB1~BUKRS = BSIK~BUKRS
                              ...
                              AND blart <> 'ZA'
                              AND AKONT NOT IN ('0000250100','0000430360')
                              AND   UMSKZ = ' '    "<> 'F'
                              AND   BUDAT <= DATE.
...
*  Title
   WA_HEADER-TYP  = 'H'.
   WA_HEADER-INFO = 'Payables Bank Report'.
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
     LV_FILE = 'ZPAYABLES_BANK_DOC_DT.TXT'.

     CONCATENATE P_FOLDER '/' LV_FILE
       INTO LV_FULLFILE.

     WRITE: / 'Payaable Bank Report for Document date Download started on', SY-DATUM, 'at', SY-UZEIT.
   ELSE.
     LV_FILE = 'ZPAYABLES_BANK_DUE_DT.TXT'.

     CONCATENATE P_FOLDER '/' LV_FILE
       INTO LV_FULLFILE.

     WRITE: / 'Payaable Bank Report for Due date Download started on', SY-DATUM, 'at', SY-UZEIT.
   ENDIF.

   OPEN DATASET LV_FULLFILE
     FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
   IF SY-SUBRC = 0.
DATA lv_string_1781 TYPE string.
DATA lv_crlf_1781 TYPE string.
lv_crlf_1781 = cl_abap_char_utilities=>cr_lf.
lv_string_1781 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_1781 lv_crlf_1781 wa_csv INTO lv_string_1781.
  CLEAR: wa_csv.
ENDLOOP.
TRANSFER lv_string_1781 TO lv_fullfile.
