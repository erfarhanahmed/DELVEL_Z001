...
ENDIF.
  SORT it_matnr .
  DELETE ADJACENT DUPLICATES FROM it_matnr COMPARING matnr posnv.
  SORT it_matnr BY vbelv posnv item_type DESCENDING.

ENDIF.
*IF it_aufm_261 IS INITIAL.
* REFRESH it_matnr[].
*ENDIF.
...
IF it_loose IS NOT INITIAL .
 ...
 IF it_aufm_new_101 IS NOT INITIAL.
  ...
  ENDIF.
   IF it_aufm_new_261 IS NOT INITIAL.
  SELECT matnr,
         spras,
         maktx
 ...
 ENDIF.
 IF it_aufm_new_261 IS NOT INITIAL.
 SORT it_aufm_new_261 BY aufnr matnr bwart.
    LOOP AT it_aufm_new_261 INTO DATA(wa_aufm_new_261).
*      on CHANGE OF wa_aufm_new_261-aufnr.

*      Where aufnr = wa_aufm_new_261-aufnr.
...
*    endon.
    ...
    READ TABLE it_aufm_new_101 INTO DATA(wa_aufm_new_101) WITH KEY aufnr = wa_aufm_new_261-aufnr.
    IF sy-subrc = 0.
